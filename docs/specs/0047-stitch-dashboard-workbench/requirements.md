...
**SUPERSEDED 2026-09-06 by spec 0051.** These are one design, described twice
from the same Stitch templates. Spec 0051 carries the merged requirements, the
difit decision, dashboard write access, and state ownership. The partial
implementation started here (the design-token stylesheet and the three touched
dashboard modules) is the foundation 0051 builds on, not work to discard.
