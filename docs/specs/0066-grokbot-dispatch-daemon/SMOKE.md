# 0066 smoke â€” overnight catch-up + dogfood
...
3. Kill/restart keeps events (smoke `smoke-overnight-20260915182245` â†’ eventId 113 path).
4. Watcher: append closed line to `dispatches.ndjson` â†’ durable event without POST.
5. `POST /dispatch` (token) accepts, spawns core CLI, status goes openedâ†’closed; watcher emits `dispatch.opened` / `dispatch.closed` (dogfood work-20260916001949-3c8cb2 â†’ eventId 116, outcome succeeded).
...
Not published. Local branch `spike/0066-dispatch-daemon` only â€” do not push without Paul asking.

## Agent skill
Shared recipe for both host models: docs/skills/checkyourvibe-cyv/SKILL.md (mirrored in the Grok Bot skill library as checkyourvibe-cyv; `.agents/` remains gitignored).
